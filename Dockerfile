...
ENV KIBANA_46_VERSION 4.6.4
ENV KIBANA_46_SHA1SUM 16b1efc124c020c9ef90b73079d55d82d284bf6d
...
# Kibana 4.6
RUN curl -fsSLO "https://download.elastic.co/kibana/kibana/kibana-${KIBANA_46_VERSION}-linux-x86_64.tar.gz" && \
    echo "${KIBANA_46_SHA1SUM}  kibana-${KIBANA_46_VERSION}-linux-x86_64.tar.gz" | sha1sum -c - && \
    tar xzf "kibana-${KIBANA_46_VERSION}-linux-x86_64.tar.gz" -C /opt && \
    mv "/opt/kibana-${KIBANA_46_VERSION}-linux-x86_64" "/opt/kibana-${KIBANA_46_VERSION}" && \
    rm "kibana-${KIBANA_46_VERSION}-linux-x86_64.tar.gz"
...
RUN rm "/opt/kibana-${KIBANA_41_VERSION}/config/kibana.yml" \
 ...
 && rm "/opt/kibana-${KIBANA_46_VERSION}/config/kibana.yml" \
...
ADD templates/opt/kibana-4.6.x/ /opt/kibana-${KIBANA_46_VERSION}/config
